...
printf "%b" "[✨" " $green" "entrypoint" "$cend" "] Starting NGINX using tini\n"
tini -- /usr/sbin/nginx \
        -c /app/configs/nginx/nginx.conf \
        -g 'daemon on;'

printf "%b" "[✨" " $green" "entrypoint" "$cend" "] Starting supercronic using tini and leaving entrypoint\n"
exec tini -- /usr/sbin/supercronic /app/configs/crontab 
